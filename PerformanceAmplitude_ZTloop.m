...
Cscale = 10;
Mscale = 10;
Lscale = 10;  % default 10

CChscale = 10; % default 10
...
if Na_clamp == 1
    disp('Na clamped')
    y0n(35) = 6.1880; %aging ZT6 6.1880 control ZT6 10.0724
end
...
if currents_flag == 1
    ...
    plot(tc*1e-3,i_sus,tc*1e-3,i_ks,tc*1e-3,i_bk,tc*1e-3,i_kach)
    ylabel('K (A/F)')
    legend('Ito','IKr','IK1','Isus','IKs','IbK','IKACh')
    xlabel('Time (s)')
...
disp(max(paraspace))
disp(min(paraspace))
disp((max(paraspace)-min(paraspace)))
disp(mean(paraspace))