...
function output = mouse_SAM_eccODEfile_aging_new(t,y,p,runType)
...
gks=0.000299*1.6; % aging Tellez 60% increase 
ecal=47.0;
kmfca=0.00035;
alpha_fca=0.021;
ecat=45.0;
enattxr=41.5761;
gsus=0.00039060;
inakmax=inakmax_multiplier*1.85*0.077*1.5; %aging 50% increase Tellez
kmnap=14.0;
kmkp=1.4;
K1ni=395.3;
K1no=1628;
...
gcal13 = 0.0030*4.0*1.5*Mscale*0.1*(1-0.31*Agingscale*0.1); %aging condition 0.69
gcat = 0.75*0.01862*Mscale*0.1*(1-0.54*Agingscale*0.1); %aging condition 0.46
gh = 0.0057*(1-0.52*Agingscale*0.1); % G_HCN aging condition 0.48
slope_gh = 16.3;
vhalf_gh = 106.8;
gkr = 1*(0.8*0.002955)*0.92; % aging Tellez 20121
v_kr = 0;
gto = 0.00492;
CCh = (1+AA_P*(BT-BT_mean))*(CChscale*0.1*1*85*cos(pi*2*(ZT-6)/24)*(0.25*Agingscale*0.1) + 265*0.25);  %aging condition 75% reduction Freelings
gkach = 0.025*2*0.14241818;
kNaCa = inaca_multiplier*5.5*0.5; %aging 0.5
Pup = 0.04*0.1*Cscale*0.7; %aging 0.7
ks = 1300000*0.2; % aging 0.2
pumpkmf = 0.00008*1.25; % c code aging to reflex PLB/SERCA ratio 25% increase
pumpkmr = 4.5; % c code
...
if m_ind == 1 % -> Updated currents
    ...
    new_If_flag = 1;
    slope_gh = 11.66; vhalf_gh = 103.2+12*Agingscale*0.1; % low (nM) ISO aging condition + 12
    gh = gh*3;
end
...
if new_If_flag == 1
    ...
    if new_If_ISO_flag == 1 % High ISO
        ...
        slope_gh = 9.15; 
        %vhalf_gh = 88.5+12; % high (uM) ISO aging +12
        vhalf_gh = 103.2 + 12*Agingscale*0.1 - scale_iso*14.7;
    end
